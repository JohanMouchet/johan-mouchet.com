...
{% macro ref(context, options) %}
	{%- spaceless %}
		<sup>
			<a id="hash-footnote-ref:{{ options.prefix ? options.prefix ~ '-' }}{{ options.id }}" class="o-footnote-ref" href="{{ context._SITE['rurl'] }}#footnote:{{ options.prefix ? options.prefix ~ '-' }}{{ options.id }}">
				{{- options.text -}}
			</a>
		</sup>
	{% endspaceless -%}
...
{% macro notes(context, options) %}
	...
	<ol class="{{ contextClass }} o-footnotes {{ additionalClasses|additionalClasses }}">
		{% for note in options.notes %}
			<li id="hash-footnote:{{ options.prefix ? options.prefix ~ '-' }}{{ note.id }}" class="{{ 'o-footnotes__note'|contextClass(contextClass) }}">
				{{ note.text|raw }}
				<a href="{{ context._SITE['rurl'] }}#footnote-ref:{{ options.prefix ? options.prefix ~ '-' }}{{ note.id }}" class="footnote-backref">&#x21a9;</a>
			</li>
		{% endfor %}
	</ol>
{% endmacro %}
