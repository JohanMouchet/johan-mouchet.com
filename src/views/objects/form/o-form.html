...
{% macro label(options) %}
	...
	<label {% if options.for %}for="{{ options.for }}"{% endif %} class="{{ contextClass }} o-form__label {{ 'o-form__label'|modifierClasses(modifierClasses) }} {{ blockLabelClass }} {{ additionalClasses|additionalClasses }}">
		{{- text|raw -}}
	</label>
{% endmacro %}
...
{% macro field(options) %}
	...
	<input {% if options.id %}id="{{ options.id }}"{% endif %} type="{{ options.type }}" name="{{ options.name }}" {% if options.value %}value="{{ options.value }}"{% endif %} class="{{ contextClass }} o-form__field {{ 'o-form__field'|modifierClasses(modifierClasses) }} o-form__field--{{ options.type }} {{ additionalClasses|additionalClasses }}" {{ attributes|attributes|raw }}>
{% endmacro %}
...
{% macro toggle(options) %}
	...
	<label class="{{ contextClass }} o-form__toggle o-form__toggle--{{ options.type }} {{ additionalClasses|additionalClasses }}">
		<input {% if options.id %}id="{{ options.id }}"{% endif %} type="{{ options.type }}" {% if options.name %}name="{{ options.name }}"{% endif %} {% if options.value %}value="{{ options.value }}"{% endif %} class="{{ 'o-form__toggle-input'|contextClass(contextClass) }}" {{ attributes|attributes|raw }}>
		<span class="{{ 'o-form__toggle-icon'|contextClass(contextClass) }}"></span>
		{%- if options.text -%}
			<span class="{{ 'o-form__toggle-text'|contextClass(contextClass) }}">{{ options.text|raw }}</span>
		{% endif -%}
	</label>
{% endmacro %}
...
{% macro file(options) %}
	...
	<label class="{{ contextClass }} o-form__file o-button js-form-file {{ additionalClasses|additionalClasses }}">
		<input {% if options.id %}id="{{ options.id }}"{% endif %} type="file" name="{{ options.name }}" class="{{ 'o-form__file-input'|contextClass(contextClass) }} js-form-file-input" data-multiple-caption="{{ options.multipleCaption ? options.multipleCaption : '{count} files selected' }}"  {{ attributes|attributes|raw }}>
		<i class="material-icons">file_upload</i>
		{% if options.text %}
			<span class="{{ 'o-form__file-text'|contextClass(contextClass) }} js-form-file-text">{{ options.text|raw }}</span>
		{% endif %}
	</label>
{% endmacro %}
...
{% macro select(options) %}
	...
	<select {% if options.id %}id="{{ options.id }}"{% endif %} name="{{ options.name }}" class="{{ contextClass }} o-form__field o-form__field--select {{ additionalClasses|additionalClasses }}" {{ attributes|attributes|raw }}>
...
{% macro textarea(options) %}
	...
	<textarea {% if options.id %}id="{{ options.id }}"{% endif %} name="{{ options.name }}" class="{{ contextClass }} o-form__field o-form__field--textarea {{ additionalClasses|additionalClasses }}" {{ attributes|attributes|raw }}>
		{{- options.text|raw -}}
	</textarea>
{% endmacro %}
...
{% macro range(options) %}
	...
	<input {% if options.id %}id="{{ options.id }}"{% endif %} type="range" name="{{ options.name }}" class="{{ contextClass }} o-form__range {{ additionalClasses|additionalClasses }}" {{ attributes|attributes|raw }}>
{% endmacro %}
