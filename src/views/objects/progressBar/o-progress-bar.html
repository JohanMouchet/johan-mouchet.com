...
{% macro default(options) %}
	...
	<div class="{{ contextClass }} {{ 'o-progress-bar'|modifierClasses(modifierClasses) }} {{ additionalClasses|additionalClasses }}">
		{% if options.label %}
			<span class="{{ 'o-progress-bar__label'|contextClass(contextClass) }}">{{ options.label|raw }}</span>
		{% endif %}
		{% if options.total %}
			<span class="{{ 'o-progress-bar__total'|contextClass(contextClass) }}">{{ options.total|raw }}</span>
		{% endif %}
		<div class="{{ 'o-progress-bar__container'|contextClass(contextClass) }}">
			<div class="{{ 'o-progress-bar__filling'|contextClass(contextClass) }} wow" style="width: {{ options.filling }}%" {% if options.tooltip %}data-progress-bar-tooltip="{{ options.tooltip|raw }}"{% endif %}></div>
		</div>
	</div>
{% endmacro %}
