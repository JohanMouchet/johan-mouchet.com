...
{% macro default(options, isSubmenu) %}
	...
	<ul class="{{ not isSubmenu ? 'o-menu' : 'o-menu__submenu' }}">
		{% for item in options.menu if item.text or item.button %}
			<li class="o-menu__item {{ item.menu ? 'o-menu__item--has-children' }} {{ item.metadata.additionalClasses|additionalClasses }}">
				{% if item.text %}
					<a class="o-menu__link {{ item.menu ? 'o-menu__submenu-heading' }}" {% if item.url %}href="{{ item.url|raw }}"{% else %}tabindex="0"{% endif %}>
						{{- item.text|raw -}}
					</a>
				{% elseif item.button %}
